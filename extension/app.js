...
function openViewer(mode){
  ...
  const state = {idx:0, hidden:false, sentences:parsed.sentences, myImgs:myImgs,
    ...
    streamMode: !!_saved.streamMode, streamTag: (typeof _saved.streamTag==='string'&&_saved.streamTag)||'content',
  ...
  function toggleSettings(){
    ...
    if(state.settingsOpen){ applyStayModeUI();applyStreamModeUI();if(state.streamMode)initStreamListen(); applyAutoPvpUI(); }
  }
  ...
  function saveSettingsToStorage(){
    try{TOP.localStorage.setItem(_stKey,JSON.stringify({streamMode:state.streamMode,streamTag:state.streamTag,fontSize:state.fontSize,dialogWidth:state.dialogWidth,dialogHeight:state.dialogHeight,glassOpacity:state.glassOpacity,toolbarScale:state.toolbarScale,inputScale:state.inputScale,pinnedBtns:state.pinnedBtns,stayMode:state.stayMode,autoPvp:state.autoPvp,imageCountOverride:state.imageCountOverride,imgMode:state.imgMode||'adaptive'}));}catch(e){}
  }
  ...
  // ═══ 流式显示引擎（streamMode）═══
  function applyStreamModeUI(){
    var sb=overlay.querySelector('#vnm-streammode-btn');
    if(!sb) return;
    sb.textContent=state.streamMode?'开':'关';
    sb.style.background=state.streamMode?'rgba(255,255,255,.16)':'rgba(255,255,255,.07)';
    sb.style.borderColor=state.streamMode?'rgba(255,255,255,.32)':'rgba(255,255,255,.14)';
    sb.style.color=state.streamMode?'rgba(255,255,255,.95)':'rgba(255,255,255,.5)';
  }
  var _stmActive=false,_stmConsumed=0,_stmBaseImg=0,_stmListening=false,_stmLast=null,_stmToast=false;
  function _stmCtx(){try{var c=TOP.SillyTavern&&TOP.SillyTavern.getContext&&TOP.SillyTavern.getContext();return (c&&c.eventSource)?c:null;}catch(e){return null;}}
  function _stmExtract(full){
    var tag=String(state.streamTag||'content').replace(/[<>\/\s]/g,'')||'content';
    var m=new RegExp('<'+tag+'[^>]*>','i').exec(full);
    if(!m)return null;
    var rest=full.slice(m.index+m[0].length);
    var e=new RegExp('</'+tag+'>','i').exec(rest);
    return e?rest.slice(0,e.index):rest;
  }
  function _stmSafe(c){
    var lc=c.toLowerCase();
    var i=lc.lastIndexOf('<image');
    if(i>=0&&lc.indexOf('</image>',i)<0)c=c.slice(0,i);
    lc=c.toLowerCase();
    var j=lc.lastIndexOf('<imgthink');
    if(j>=0&&lc.indexOf('</imgthink>',j)<0)c=c.slice(0,j);
    var k=c.lastIndexOf('image###');
    if(k>=0&&c.indexOf('###',k+8)<0)c=c.slice(0,k);
    var cnt=(c.match(/:::/g)||[]).length;
    if(cnt%2===1)c=c.slice(0,c.lastIndexOf(':::'));
    return c;
  }
  function _stmVoice(seg){
    var txt=seg.text||'';
    var m=/\[VoiceTag\s*[:：]\s*([^\]]+)\]/i.exec(txt);
    if(m){
      var parts=m[1].trim().split(/\s*[:：]\s*/);
      if(parts.length>=3) seg.voice={name:parts[0].trim(),lang:parts[1].trim(),text:parts.slice(2).join(':').trim()};
      else if(parts.length===2) seg.voice={name:parts[0].trim(),lang:'',text:parts[1].trim()};
      seg.text=txt.replace(/\[VoiceTag\s*[:：]\s*[^\]]+\]/gi,'').replace(/^\s+/,'');
    }
    return seg;
  }
  function _stmFeed(content,final){
    _stmLast=content;
    var parsed;
    try{parsed=parseRawSource(_stmSafe(content));}catch(e){return;}
    var segs=(parsed.sentences||[]).filter(function(x){var t=x.text||'';return t&&!/^:::/.test(t)&&!/^(MainText|Status)\s*\|/.test(t);});
    var avail=final?segs.length:Math.max(0,segs.length-1);
    if(avail<=_stmConsumed)return;
    for(var i=_stmConsumed;i<avail;i++){
      var sg=_stmVoice(segs[i]);
      state.sentences.push({text:sg.text,voice:sg.voice||null,imgIdx:_stmBaseImg+(segs[i].imgIdx||0)});
    }
    _stmConsumed=avail;
    if(parsed.imageCount)state.imageCount=Math.max(state.imageCount,_stmBaseImg+parsed.imageCount);
    if(!_stmToast){_stmToast=true;showToast('流式正文已开始，点击继续 →',2400);}
    try{render();}catch(e){}
    try{if(state.myImgs.length<state.imageCount&&!state.pollTimer)startPolling();}catch(e){}
  }
  function initStreamListen(){
    if(_stmListening)return;
    var c=_stmCtx(); if(!c)return;
    var ES=c.eventSource,ET=c.eventTypes||TOP.event_types||{};
    if(ET.STREAM_TOKEN_RECEIVED===undefined||ET.GENERATION_STARTED===undefined)return;
    var isLast=function(){try{var l=getVnmMes();return l.length&&l[l.length-1]===getMyMesElement();}catch(e){return false;}};
    ES.on(ET.GENERATION_STARTED,function(){
      if(!state.streamMode||!overlay.isConnected||!isLast())return;
      _stmActive=true;_stmConsumed=0;_stmBaseImg=state.imageCount;_stmLast=null;_stmToast=false;
    });
    ES.on(ET.STREAM_TOKEN_RECEIVED,function(full){
      if(!_stmActive||!state.streamMode||!overlay.isConnected)return;
      if(typeof full!=='string')return;
      var content=_stmExtract(full);
      if(content==null)return;
      _stmFeed(content,false);
    });
    ES.on(ET.GENERATION_ENDED,function(){
      if(!_stmActive)return;
      _stmActive=false;
      if(_stmLast!=null)_stmFeed(_stmLast,true);
    });
    _stmListening=true;
  }
  function showSentState(){
    ...
    // ── 事件驱动完成判定（优先，免疫百宝箱等 DOM 优化器）──
    var _stc=null,_es=null,_et={};
    try{_stc=TOP.SillyTavern&&TOP.SillyTavern.getContext&&TOP.SillyTavern.getContext();_es=_stc&&_stc.eventSource;_et=(_stc&&_stc.eventTypes)||TOP.event_types||{};}catch(e){}
    if(_es&&_et.GENERATION_ENDED!==undefined){
      var _done=false;
      var _off=function(ev,fn){try{if(_es.removeListener)_es.removeListener(ev,fn);else if(_es.off)_es.off(ev,fn);}catch(e){}};
      var _onTok=function(){if(txt)txt.textContent='AI 回复中…';};
      var _cleanup=function(){_off(_et.STREAM_TOKEN_RECEIVED,_onTok);_off(_et.GENERATION_ENDED,_onEnd);_off(_et.GENERATION_STOPPED,_onStop);};
      var _finish=function(){
        if(_done)return;_done=true;_cleanup();hideSentState();
        var _rc=function(){updateTurnCornerBtn();var l=getVnmMes();var c2=getMyMesElement();var i2=l.indexOf(c2);if(i2>=0&&i2<l.length-1){showToast('新对话已就绪 →',2500);return true;}return false;};
        TOP.setTimeout(function(){if(_rc())return;TOP.setTimeout(function(){if(_rc())return;TOP.setTimeout(_rc,3000);},1400);},600);
      };
      var _onEnd=function(n){if(typeof n==='number'&&n<=mesCount)return;_finish();};
      var _onStop=function(){_finish();};
      if(_et.STREAM_TOKEN_RECEIVED!==undefined)_es.on(_et.STREAM_TOKEN_RECEIVED,_onTok);
      _es.on(_et.GENERATION_ENDED,_onEnd);
      if(_et.GENERATION_STOPPED!==undefined)_es.on(_et.GENERATION_STOPPED,_onStop);
      TOP.setTimeout(function(){if(!_done){_cleanup();hideSentState();}},600000);
      return;
    }
  ...
  setTimeout(function(){
    ...
    try{var _sti=sp.querySelector('#vnm-streamtag-inp'); if(_sti){_sti.value=state.streamTag||'content'; _sti.addEventListener('change',function(){state.streamTag=(_sti.value||'content').trim()||'content'; saveSettingsToStorage();}); _sti.addEventListener('click',function(e){e.stopPropagation();});}}catch(e){}
    sp.addEventListener('click',function(e){
      ...
      else if(type==='streammode'){ state.streamMode=!state.streamMode; applyStreamModeUI(); saveSettingsToStorage(); if(state.streamMode) initStreamListen(); }
